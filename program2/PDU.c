...
int sendPDU(int clientSocket, uint8_t *dataBuffer, int payloadLength) {
  ...
  memcpy(PDU + 2, dataBuffer, payloadLength); // Copy data into the PDU's
                                              // payload

  return safeSend(clientSocket, PDU, payloadLength + 2,
                  0); // Returns the # of bytes sent
}
...
int recvPDU(int socketNumber, uint8_t *dataBuffer, int bufferSize) {
  ...
  if (bytesReceived == 0) { // Connection is closed
    ...
    if (bufferSize <
        ntohs(PDULength)) { // ERROR DETECTION: Buffer size too small for PDU
      perror("Buffer size < PDU length");
      return -1;
    }

    hostOrder_payloadLength =
        ntohs(PDULength) - 2; // Payload length = PDU length - header

    return safeRecv(
        socketNumber, dataBuffer + 2, hostOrder_payloadLength,
        MSG_WAITALL); // Returns the # of bytes received excluding header
